...
def MLE_Euler(HyB,X,N,dt,tempF):
    '''
    input agruments:
...
# integrate to get rid of transient behaviour,sothat it settled on the attractor of the dynamics
    j=1
    while j<=t_trans:
        datos0=datos0+dt*HyB(datos0,j*dt,tempF)
        j+=1
    print("finished adaptation simulation")
...
# integrate both orbits of  networks  by a time dt:
    while i<N: # N is integration steps of times
        datos[:,:,i]=datos[:,:,i-1]+dt*HyB(datos[:,:,i-1],i*dt,tempF);# unperturbed systems
        datosd0[:,:,i]=datosd0[:,:,i-1]+dt*HyB(datosd0[:,:,i-1],i*dt,tempF); # slightly pertured  systems
